...
function (\cf{summarize}) of type \cf{[e] -> [e]} for each RDT. We utilize this
function to ensure that the state of an object remains summarized in each
replica. The summarization function for the bank account is {\footnotesize
\cf{summarize ctxt = [Deposit \$ sel1 \$ getBalance ctxt ()]}}.
