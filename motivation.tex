...
Here, \cf{getBalance} is a read-only operation,
...
\noindent To prevent \cf{getBalance} from ever showing negative balance, it is
necessary to prevent the scenario depicted in
fig.~\ref{fig:negativeBalanceAnomaly}. Let $\cureff$ stand for the effect
emitted by the \cf{getBalance} operation. If the effect ($b$) of a withdraw
...
\cf{getBalance} operation precisely captures this application-level consistency
requirement:
...
\qquad\qquad (\vis{a}{b} \wedge \vis{b}{\cureff} \Rightarrow \vis{a}{\cureff})
\end{array}
\end{smathpar}

\noindent To prevent the missing update anomaly described in
fig.~\ref{fig:missingUpdateAnomaly}, it is necessary for a \cf{getBalance}
operation on a bank account to witness the effects of all previous \cf{deposit}
and \cf{withdraw} operations performed on the same bank account in the same
session. We can conceive an alternative contract ($\cv_{gb}^2$) for
\cf{getBalance} to express this consistency requirement:

\vspace{-1em}
\begin{smathpar}
\begin{array}{l}
\forall (c: \rcf{deposit} \vee \rcf{withdraw}). \\
\qquad \qquad((\soZ \cap \sameobjZ) (c,\cureff) \Rightarrow \vis{c}{\cureff})
\end{array}
\end{smathpar}
...
above contract ($\cv_{gb}^2$) says that if a \cf{deposit} or a \cf{withdraw}
operation precedes a \cf{getBalance} operation in session order, and is applied
on the same object as the \cf{getBalance} operation, then it must be the case
that the \cf{getBalance} operation witnesses the effects of the preceding
operations.

\noindent To final contract ($\cv_{gb}$) of the \cf{getBalance} operation is
...
\noindent Intuitively, this prevents both the \cf{getBalance} anomalies
