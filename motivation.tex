...
Each object is associated with a set of \emph{operations}. The clients
...
This model admits all the inconsistencies associated with eventual
...
object provides three operations: \cf{deposit}, \cf{withdraw} and
\cf{getBalance}, with the assumption that the withdraw fails if the account
...
the initial balance being zero. Session 1 performs a \cf{deposit} of 100, followed by a
...
constraint. A subsequent \cf{getBalance} operation, that happens to witness all the
previous operations,  would report a negative balance.
...
constraint from being violated, a strongly consistent \cf{withdraw} itself
is not sufficient to prevent \cf{getBalance} from ever reporting a negative
...
\cf{withdraw}, and a \cf{getBalance} operation, respectively, on the same
bank account object. As before, assume that the initial balance in the
account is zero. As the \cf{vis} edge indicates, operation \cf{withdraw(50)}
in session 2, can witnesses the effects of \cf{deposit(100)} from session 1,
leading it to rightfully conclude that the current balance is
50. Consequently, \cf{withdraw(50)} is performed safely. The effect of this
\cf{withdraw} operation subsequently becomes visible to the \cf{getBalance}
in Session 3.  However, because \cf{getBalance} only witnesses the effect of
the \cf{withdraw} operation from session 2, but not the \cf{deposit} from
...
\cf{withdraw} operation performed in the same session, possibly because it
was served by a replica that has not yet merged the \cf{withdraw} effect,
served by a different replica. This anomaly leads the user to incorrectly
conclude that the \cf{withdraw} operation failed to go through.
...
%%  a strongly consistent operation is
%% significantly expensive (Figure~\ref{grf:BankAcc_Th_vs_Lat} \KC{Should I
%%   include the numbers here? If yes, I'll expand the text.}), both in terms
%% of availability and monetary overheads~\cite{DynamoDB,Pileus}, we ideally
%% would not want to request strong consistency, unless there are no
...
constraint, the \cf{withdraw} operation must be strongly consistent.  That
is, given two \cf{withdraw} operations $a$ and $b$, either $a$ is visible to
...
\end{array}
\end{smathpar}
Here, $\cureff$ stands for the effect emitted by the \cf{withdraw}
operation. The syntax $a : \rcf{withdraw}$ states that $a$ is an effect
which was emitted by a \cf{withdraw} operation i.e.,
...
which was emitted by a \cf{withdraw} operation, it is the case that $a =
\cureff$ or $a$ is visible to $\cureff$, or vice versa. Any execution on a
bank account object that preserves the above contract for a \cf{withdraw}
operation is said to be derived from a correct implementation of
\cf{withdraw}.

For \cf{getBalance}, we construct the following contract:
\begin{smathpar}
\begin{array}{l}
...
\cf{getBalance}. This prevents negative balance anomalies. The expression $c
: \rcf{deposit} \vee \rcf{withdraw}$ states that $c$ is an effect that was
emitted either by a \cf{deposit} or a \cf{withdraw} operation.  Additonally,
...
\cf{deposit} or a \cf{withdraw} precedes a \cf{getBalance} operation in
...
restrictions on when and how a \cf{deposit} operation can execute, its
...
We devise a static approach instead.  Contracts are analyzed with the help
