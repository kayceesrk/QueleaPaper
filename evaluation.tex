...
In this section, we evaluate \name programs, report their contract profile and
illustrate the performance benefits of fine-grained consistency classification
on operations and transactions. We also evaluate on the impact of the
summarization. We implemented the following applications, which includes
...
For our performance evaluation, we deploy \name applications in \emph{clusters},
where each cluster is composed of 5 fully replicated Cassandra replicas within
the same datacenter. We instantiate one shim layer node for every Cassandra
replica, and place it on the same VM as the Cassandra replica. Clients are
instantiated on the same data center as the store, and run transactions. We
deploy the each node in the cluster on \cf{c3.4xlarge} Amazon EC2 instances.
Our shim layer nodes are multi-threaded, and we allocate 8 CPUs (out of 16
available) for each shim layer node. The clients also run on \cf{c2.4xlarge}
instances. We call this \cf{1DC} configuration. For our geo-distributed
experiments (\cf{2DC}), we instantiate 2 clusters, each with 5 nodes, and place
the clusters on US-east (Virginia) and US-west (Oregon). The average
inter-region latency was 85ms.

\begin{figure*}[t]
  \centering
  \subfigure[Latency]{\label{grf:BA-lat}\includegraphics[width=0.32\textwidth]{graphs/BA-lat.pdf}}
  \subfigure[Throughput]{\label{grf:BA-tp}\includegraphics[width=0.32\textwidth]{graphs/BA-tp.pdf}}
  \subfigure[Throughput vs. Latency]{\label{grf:BA-tp-vs-lat}\includegraphics[width=0.32\textwidth]{graphs/BA-tp-vs-lat.pdf}}
...
\begin{figure*}[t]
  \centering
  \subfigure[Latency]{\label{grf:LWW-txn-lat}\includegraphics[width=0.32\textwidth]{graphs/LWW-txn-lat.pdf}}
  \subfigure[Throughput]{\label{grf:LWW-txn-tp}\includegraphics[width=0.32\textwidth]{graphs/LWW-txn-tp.pdf}}
  \subfigure[Throughput vs. Latency]{\label{grf:LWW-txn-tp-vs-lat}\includegraphics[width=0.32\textwidth]{graphs/LWW-txn-tp-vs-lat.pdf}}
...
\begin{figure*}[t]
  \centering
  \subfigure[Latency]{\label{grf:rubis-lat}\includegraphics[width=0.32\textwidth]{graphs/Rubis-lat.pdf}}
  \subfigure[Throughput]{\label{grf:rubis-tp}\includegraphics[width=0.32\textwidth]{graphs/Rubis-tp.pdf}}
  \subfigure[Throughput vs. Latency]{\label{grf:Rubis-tp-vs-lat}\includegraphics[width=0.32\textwidth]{graphs/Rubis-tp-vs-lat.pdf}}
