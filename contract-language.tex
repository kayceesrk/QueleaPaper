...
simply the name of the operation (eg: \cf{withdraw}) that induced the
...
$\Mod{\cv}$ be the set of all executions under which contract $\cv$ is satisfied. If
...
decidable~\cite{epr}.  Completeness of the logic~\cite{completeness} allows
us to claim that $\cv_{st} \models_m \cv_{op}$ if and only if $\cv_{st} \Rightarrow \cv_{op}$.
...
\cf{deposit} is an eventually consistent operation. While eventually consistent data
...
list all the axioms somewhere.}  The following definition thus precisely
