...
decidable~\cite{epr}.  Completeness of the logic~\cite{completeness} allows us
to claim that $\cv_{st} \models_m \cv_{op}$ if and only if $\cv_{st}
...
prover. In this case, we say that $\cv_{op}$ is \emph{weaker than} $\cv_{st}$
(written $\cv_{op} \le \cv_{st}$).
...
\item \textbf{Causal consistency}: Operations with causally consistent
	...
	in both $r_1$ and $r_2$.  \cf{getBalance} operation is classified as a
...
presented formally in Figure~\ref{sem:classify}; here, $\Delta$ captures
assumptions about the nature of primitive relations, such as $\Rvis$ and $\Rso$
are irreflexive, and the happens-before relation $\hbZ$ is acyclic, preventing
thin-air reads. \KC{Ideally, we need to list all the axioms somewhere.}  The
following definition thus precisely captures our intuition about contract
strength in terms of these rules:
