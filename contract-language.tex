...
\noindent The $\Delta$ referred in the above definition is a conjunction of
following assumptions about the nature of primitive relations:

\begin{itemize}
\item $\hboZ$ is acyclic:
  \begin{itemize}
    \item $\forall a. \neg\hboZ(a,a)$
  \end{itemize}
\item $\visZ$ relates actions on the same object:
  \begin{itemize}
    \item $\forall a,b. \visZ(a,b) \Rightarrow \sameobjZ(a,b)$
  \end{itemize}
\item $\soZ$ is a transitive relation:
  \begin{itemize}
    \item $\forall a,b,c. \soZ(a,b) \conj \soZ(b,c) \Rightarrow \soZ(a,c)$
  \end{itemize}
\item $\sameobjZ$ is an equivalence relation:
  \begin{itemize}
    \item $\forall a. \sameobjZ(a,a)$
    \item $\forall a,b. \sameobjZ(a,b) \Rightarrow \sameobjZ(b,a)$
    \item $\forall a,b,c. \soZ(a,b) \conj \soZ(b,c) \Rightarrow \soZ(a,c)$
  \end{itemize}
\end{itemize}

\noindent A \emph{well-formed} axiomatic execution ($\E$) is expected
to satisfy these assumptions:

\begin{definition}
An axiomatic execution $\E$ is said to be well-formed if and only if
$\E \models \Delta$.
\end{definition}

\noindent If the contract ($\cv_{op}$) of an operation ($\mathit{op}$)
is \emph{weaker than} a store contract ($\cv_{st}$), then constraints
expressed by the former are implied by guarantees provided by the
latter. The completeness of first-order logic allows us to assert that
any well-formed execution ($\E$) that satisfies $\cv_{st}$ (i.e.,
$\E\models \cv_{st}$) also satisfies $\cv_{op}$ (i.e., $\E \models
\cv_{op}$). Consequently, it is safe to execute operation
$\mathit{op}$ under a store consistency level captured by $\cv_{st}$.
...
\begin{proof}
  ...
  \begin{smathpar}
  \begin{array}{cr}
    \Delta \vdash \cv_\tau \Rightarrow \cv & H\npp\\
  \end{array}
  \end{smathpar}
  Since $\cureff$ occurs free in $\cv$ and $\cv_\tau$, but not in
  $\Delta$, we introduce quantifier to bind $\cureff$:
  \begin{smathpar}
  \begin{array}{cr}
